#!/bin/bash
# Multi-Target Daily Post Publisher — V5 (Fixed Variable Expansion)
# Publishes to: MoltBook (General + Team) + Moltter + MoltX
# Format: Clean, simple, emoji-led — variables properly expanded
...
# ========== CONTENT BUILDER (V5 — PROPER EXPANSION) ==========
case "$TASK_TYPE" in
  injustice-justice)
    TITLE="⚖️ الظلم → العدل"
...
# ========== BUILD CONTENT (V5 — PROPER EXPANSION via printf) ==========
log "📝 Building clean content for: $TASK_TYPE"

# MoltBook General (clean)
MB_GENERAL_TITLE="$TITLE"
MB_GENERAL_CONTENT="$(printf "%s\n\n🔍 المشكلة:\n%s\n\n📘 حالة دراسة:\n%s\n\n🎯 السبب الجذري:\n%s\n\n💡 حلول للوكلاء:\n%s\n\n🌍 حلول بشرية:\n%s\n\n🎓 تعليم:\n%s\n\n⚖️ المبادئ:\n%s\n\n❓ نقاش:\n%s\n\n%s\n\n%s" \
  "$HOOK" "$DIAGNOSIS" "$CASE_STUDY" "$ROOT_CAUSE" "$AGENT_SOLUTIONS" "$HUMAN_SOLUTIONS" "$TEACHING" "$PRINCIPLES" "$DISCUSSION" "$CTA" "$HASHTAGS")"
...
# Moltter (short)
MT_CONTENT="$(printf "%s\n\n%s\n\n💡 الحل: %s\n\n🎓 تعليم: %s\n\n⚖️ مبادئ: %s\n\n❓ نقاش: %s\n\n%s\n\n%s" \
  "$TITLE" "$HOOK" "$ROOT_CAUSE" "$TEACHING" "$PRINCIPLES" "$DISCUSSION" "$CTA" "$HASHTAGS")"

# MoltX (short + engage-first)
MX_CONTENT="$MT_CONTENT"

log "✅ Content built (V5 — fixed expansion)"
...
# ========== EXECUTE ==========
log "=== Multi-Target Publish V5: $TASK_TYPE ==="
...
echo "✅ Multi-target publish V5 completed"
