...
# ============================================
# Mission completeness check — enhanced
# Returns 0 if mission is considered fully published today.
# Criteria:
#  1) publish_run entry with status="full_success" for today, OR
#  2) All three platforms (moltx, moltbook, moltter) have a successful post_publish entry today
# ============================================
is_mission_complete() {
  local mission="$1"
  # Use jq if available and ledger exists
  if [ -f "$LEDGER_FILE" ] && command -v jq &>/dev/null; then
    # First: check for full_success publish_run today
    if grep -q '^\{"ts":' "$LEDGER_FILE" && jq -e --arg m "$mission" --arg today "$TODAY" 'select(.type=="publish_run" and .payload.mission==$m and .payload.status=="full_success" and (.ts // "" | startswith($today)))' "$LEDGER_FILE" > /dev/null; then
      return 0
    fi
    # Second: check all three platforms have successful post_publish today
    local platforms=("moltx" "moltbook" "moltter")
    for p in "${platforms[@]}"; do
      if ! jq -e --arg m "$mission" --arg p "$p" --arg today "$TODAY" 'select(.type=="post_publish" and .payload.mission==$m and .payload.platform==$p and .payload.success==true and (.ts // "" | startswith($today)))' "$LEDGER_FILE" > /dev/null; then
        return 1
      fi
    done
    return 0
  else
    # Fallback: check publish_log for "نشر: $mission" (less reliable but acceptable)
    if grep -Fq "نشر: $mission" "${WORKSPACE}/memory/publish_log_${TODAY}.md" 2>/dev/null; then
      return 0
    else
      return 1
    fi
...
log "=== Continuity 30min Check Start (v3.1 - Smart-Repair) ==="
...
for m in "${DAILY_MISSION_NAMES[@]}"; do
  ...
  if [ "$CURRENT_MINUTES" -ge "$sched_minutes" ]; then
    ...
    # Check if mission already fully published today using ledger (publish_run full_success OR all platforms' post_publish)
    if is_mission_complete "$m"; then
      : # mission complete
    else
      missing_missions+=("$m")
    fi
